...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', first_name, id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	public function delete_user($id)
	{
		...
		{
			$this->notificacion->add_error('No fue posible eliminar el usuario.');
		    return FALSE;
		}
		...
		$this->notificacion->add_message('Usuario eliminado con &eacute;xito.');
		return TRUE;
	}
	...
	**/
	...
	public function name($username)
	{
        ...
        $first_name = $this->db->get()->result_array();
        return $first_name;
	}
